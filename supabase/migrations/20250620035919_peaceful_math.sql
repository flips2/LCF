/*
  # Complete Database Schema with Session Types

  1. New Tables
    - `trading_sessions` - User trading sessions with session_type
    - `trades` - Individual trades with enhanced fields for both Forex and Crypto
...
-- Create trading_sessions table with session_type
CREATE TABLE IF NOT EXISTS public.trading_sessions (
  ...
  session_type text NOT NULL DEFAULT 'Forex' CHECK (session_type IN ('Forex', 'Crypto')),
...
-- Create trades table with all enhanced fields for both Forex and Crypto
CREATE TABLE IF NOT EXISTS public.trades (
  ...
  futures_symbol text NULL,
  margin_mode text NULL,
  avg_entry_price numeric NULL,
  avg_close_price numeric NULL,
  direction text NULL CHECK (direction IS NULL OR direction = ANY (ARRAY['Long'::text, 'Short'::text])),
  margin_adjustment_history text NULL,
  closing_quantity numeric NULL,
